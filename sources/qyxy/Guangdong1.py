...
class Crawler(object):
    ...
    def crawl_ind_comm_pub_pages(self, url, post_data={}):
        ...
        try:
            ...
            sub_json_dict['ind_comm_pub_reg_shareholder'] = dict_jiben[u'出资人及出资信息'] if dict_jiben.has_key(
                u'出资人及出资信息') else []
    ...
    def crawl_other_dept_pub_pages(self, url):
        ...
        try:
            ...
            sub_json_dict["other_dept_pub_administration_sanction"] = xk[u'行政处罚结果信息'] if xk.has_key(u'行政处罚结果信息') else [
            ]    # 行政处罚信息
        except Exception as e:
...
class Analyze(object):
    ...
    def get_record_table_columns_by_tr(self, tr_tag, table_name):
        ...
        try:
            ...
            settings.logger.error(u'exception occured in get_table_columns_by_tr, except_type = %s, table_name = %s' %
                                  (type(e), table_name))
        finally:
            return columns
    ...
    def parse_table_1(self, bs_table, columns=[], post_data={}, table_name=""):
        ...
        try:
            ...
            if columns:
                ...
                next_dicts = {
                    u"主要人员信息":
                    "http://gsxt.gzaic.gov.cn:7001/search/search!staffListShow?entityVo.pageSize=10&entityVo.curPage=%d&entityVo.pripid=%s",
                    u"分支机构信息":
                    "http://gsxt.gzaic.gov.cn:7001/search/search!branchListShow?_=1451527284000&entityVo.curPage=%d&entityVo.pripid=%s",
                    u"出资人及出资信息":
                    "http://gsxt.gzaic.gov.cn:7001/search/search!investorListShow?_=1451527307501&entityVo.curPage=%d&entityVo.pripid=%s",
                    u"变更信息":
                    "http://gsxt.gzaic.gov.cn:7001/search/search!changeListShow?_=1451527321304&entityVo.curPage=%d&entityVo.pripid=%s",
                }
                                ...
                                if td.find('a'):
                                    ...
                                    if next_url:
                                        ...
                                        if table_name == u'基本信息':
                                            page_data = self.parse_ent_pub_annual_report_page_1(detail_page['page'],
                                                                                                table_name + '_detail')
