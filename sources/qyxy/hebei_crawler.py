...
class HebeiCrawler(object):
    ...
    def __init__(self, json_restore_path):
        self.CR = CR.CaptchaRecognition("hebei")
    ...
    def crawl_page_captcha(self, url_search, url_Captcha, url_CheckCode,url_showInfo,  textfield= '130000000021709'):
        ...
        while True:
            ...
            if r.status_code != 200:
                settings.logger.error(u"Something wrong when getting the Captcha url:%s , status_code=%d", url_Captcha+ str(random.random()), r.status_code)
                return
            ...
            if self.save_captcha(r.content):
                ...
                settings.logger.info("cracked captcha is %d"%(datas['captcha']) )
        ...
        for comstr in coms:
            ...
            if m_invstr:
                ...
                try:
                    ...
                    settings.logger.error(u"exception : %s"%(type(e)))
                item[u'详情'] = item_list
                Item.append(item)
            else:
                settings.logger.error(u"There is no company, continue!")
        return Item
