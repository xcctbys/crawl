...
class ShaanxiCrawler(object):
    ...
    def crawl_page_main(self):
        ...
        try:
            for ent in self.ents:
                ...
                sub_json_dict.update(self.crawl_ent_pub_pages(url, {'maent.pripid': pripid,
                                                                    'random': int(time.time())}))
                sub_json_dict.update(self.crawl_other_dept_pub_pages(url, {'maent.pripid': pripid,
                                                                           'random': int(time.time())}))
                sub_json_dict.update(self.crawl_judical_assist_pub_pages(url, {'maent.pripid': pripid,
                                                                               'random': int(time.time())}))
    ...
    def crawl_ent_pub_pages(self, url="", post_data={}):
        ...
        try:
            ...
            sub_json_dict['ent_pub_shareholder_capital_contribution'] = tzr[u'股东及出资信息'] if tzr.has_key(
                u'股东及出资信息') else []
        ...
        if bs4_tag.has_attr('href') and (bs4_tag['href'] != '###' and bs4_tag['href'] != '#' and
            ...
            if re.compile(r'showRyxx').search(astr):
                ...
                if m:
                    ...
                    return urls[
                        'host'] + "ztxy.do?method=tzrCzxxDetial&maent.xh=" + ryId + "&maent.pripid=" + nbxh + "&random=" + str(
                            int(time.time()))
            elif re.compile(r'doNdbg').search(astr):
                m = re.compile(r'\d+').search(astr)
                if m:
                    year = eval(m.group())
                    return urls['host'] + "ztxy.do?method=ndbgDetail&maent.pripid=" + self.pripid + "&maent.nd=" + str(
                        year) + "&random=" + str(int(time.time()))
            elif re.compile(r'doXkxkDetail').search(astr):
                m = re.findall(r'(\'.*?\')', astr)
                if m:
                    nbxh, xh, lx = [s.strip("'") for s in m]
                    return urls[
                        'host'] + "ztxy.do?method=doXkxkDetail&maent.pripid=" + nbxh + "&maent.xh=" + xh + "&maent.lx=" + lx + "&random=" + str(
                            int(time.time()))
                ...
                if m:
                    ...
                    return urls[
                        'host'] + "ztxy.do?method=zscqDetail&maent.pripid=" + self.pripid + "&maent.xh=" + xh + "&maent.lx=" + lx + "&random=" + str(
                            int(time.time()))
                ...
                if m:
                    ...
                    return urls[
                        'host'] + "ztxy.do?method=gqczxxZxDetail&maent.pripid=" + self.pripid + "&maent.lx=X&maent.xh=" + licid + "&random=" + str(
                            int(time.time()))
                ...
                if m:
                    licid = [s.strip("'") for s in m]
                    return urls[
                        'host'] + "ztxy.do?method=gqczxxZxDetail&maent.pripid=" + self.pripid + "&maent.lx=C&maent.xh=" + licid + "&random=" + str(
                            int(time.time()))
                ...
                if m:
                    licid = [s.strip("'") for s in m]
                    return urls[
                        'host'] + "ztxy.do?method=gqczxxDetail&maent.pripid=" + self.pripid + "&maent.xh=" + licid + "&random=" + str(
                            int(time.time()))
                ...
                if m:
                    ...
                    return urls[
                        'host'] + "ztxy.do?method=doXzfyDetail&maent.pripid=" + nbxh + "&maent.xh=" + ajbh + "&random=" + str(
                            int(time.time()))
                ...
                if m:
                    ...
                    return urls[
                        'host'] + "ztxy.do?method=doGqdjbgDetail&maent.pripid=" + self.pripid + "&maent.xh=" + ids + "&random=" + str(
                            int(time.time()))
                ...
                if m:
                    ids = [s.strip("'") for s in m]
                    return urls[
                        'host'] + "ztxy.do?method=doGqdjDetail&maent.pripid=" + self.pripid + "&maent.xh=" + ids + "&random=" + str(
                            int(time.time()))
                ...
                if m:
                    ...
                    return urls[
                        'host'] + "ztxy.do?method=doXzxkDetail&maent.pripid=" + self.pripid + "&maent.xh=" + xh + "&random=" + str(
                            int(time.time()))
                ...
                if m:
                    xh = [s.strip("'") for s in m]
                    return urls[
                        'host'] + "ztxy.do?method=doXzcfDetail&maent.pripid=" + self.pripid + "&maent.xh=" + xh + "&random=" + str(
                            int(time.time()))
                ...
                if m:
                    ...
                    return urls[
                        'host'] + "ztxy.do?method=qtgsxxDetail&newsid=" + newsid + "&flag=" + flag + "&random=" + str(
                            int(time.time()))
            else:
                pass
