...
class JiangsuCrawler(Crawler):
    ...
    def crawl_ind_comm_pub_pages(self):
        ...
        if not self.parser.ind_comm_pub_skeleton_built:
            ...
            if not page:
                settings.logger.error('crawl ind comm pub skeleton failed!')
                return False
    ...
    def crawl_ent_pub_pages(self):
        ...
        if not self.parser.ent_pub_skeleton_built:
            ...
            if not page:
                settings.logger.error('crawl ent pub skeleton failed!')
                return False
        ...
        if not self.parser.annual_report_skeleton_built:
            ...
            if not page:
                settings.logger.error('crawl annual report skeleton failed!')
                return False
    ...
    def crawl_other_dept_pub_pages(self):
        ...
        if not self.parser.other_dept_pub_skeleton_built:
            ...
            if not page:
                settings.logger.error('crawl other dept pub skeleton failed!')
                return False
    ...
    def get_pages(self, url, post_data):
        ...
        if resp.status_code != 200:
            settings.logger.error('get all pages of a section failed!')
            return
        else:
    ...
    def crawl_skeleton_page(self, name):
        ...
        resp = self.reqst.post(url, data=post_data)
        if resp.status_code != 200:
            settings.logger.error('crawl %s page failed, error code = %d' % (name, resp.status_code))
            return False
        return resp.content
