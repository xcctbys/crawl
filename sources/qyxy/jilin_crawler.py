...
class JilinCrawler(object):
    ...
    def __init__(self, json_restore_path):
        self.CR = CR.CaptchaRecognition("jilin")
    ...
    def crawl_page_captcha(self, url_search, url_Captcha, url_CheckCode, textfield=''):
        ...
        while True:
            ...
            r = self.requests.get(url_Captcha,
                                  cookies={'ROBOTCOOKIEID': self.ROBOTCOOKIEID},
                                  headers={'Referer': "http://211.141.74.198:8081/aiccips/"})
    ...
    def parse_table_jingyingyichang(self, bs_table, table_name, page, post_data):
        ...
        sub_json_list = []
        try:
            settings.logger.info(u"parse table 经营异常信息 ")
    ...
    def parse_table_xingzhengchufa(self, bs_table, table_name, page, post_data):
        ...
        sub_json_list = []
        try:
            settings.logger.info(u"parse table 行政处罚信息 ")
            columns = self.get_columns_of_record_table(bs_table, page, table_name)
            titles = [column[0] for column in columns]
    ...
    def parse_table_yanzhongweifa(self, bs_table, table_name, page, post_data):
        ...
        sub_json_list = []
        try:
            settings.logger.info(u"parse table 严重违法信息")
            columns = self.get_columns_of_record_table(bs_table, page, table_name)
            titles = [column[0] for column in columns]
    ...
    def parse_table_guquanchuzhi(self, bs_table, table_name, page, post_data):
        ...
        sub_json_list = []
        try:
            settings.logger.info(u"parse table 股权出质 ")
            columns = self.get_columns_of_record_table(bs_table, page, table_name)
            titles = [column[0] for column in columns]
    ...
    def parse_table_dongchandiya(self, bs_table, table_name, page, post_data):
        ...
        sub_json_list = []
        try:
            settings.logger.info(u"parse table 动产抵押登记信息")
            columns = self.get_columns_of_record_table(bs_table, page, table_name)
            titles = [column[0] for column in columns]
    ...
    def parse_table_qygs_xinzhengchufa(self, bs_table, table_name, page, post_data):
        sub_json_list = []
        try:
            settings.logger.info(u"parse qygs table 行政处罚信息")
            columns = self.get_columns_of_record_table(bs_table, page, table_name)
            titles = [column[0] for column in columns]
            url = urls['host'] + "qygsjsxxxzcfxx"
            res = self.crawl_page_by_url_post(
                url, post_data,
                {'X-CSRF-TOKEN': self.csrf,
                 'X-Requested-With': 'XMLHttpRequest',
                 'Referer': "http://211.141.74.198:8081/aiccips/pub/qygsdetail/1219/" + self.encrpripid})['page']
            ls = json.loads(res)
            for i, l in enumerate(ls):
    ...
    def parse_table_qygs_zhishichanquan(self, bs_table, table_name, page, post_data):
        ...
        sub_json_list = []
        try:
            settings.logger.info(u"parse qygs table 知识产权出质登记信息")
            columns = self.get_columns_of_record_table(bs_table, page, table_name)
            titles = [column[0] for column in columns]
    ...
    def parse_table_qygs_xinzhengxuke(self, bs_table, table_name, page, post_data):
        sub_json_list = []
        try:
            settings.logger.info(u"parse qygs table 行政许可信息")
            columns = self.get_columns_of_record_table(bs_table, page, table_name)
            titles = [column[0] for column in columns]
    ...
    def parse_table_qygs_guquan(self, bs_table, table_name, page, post_data):
        ...
        sub_json_list = []
        try:
            settings.logger.info(u"parse qygs table 股权变更信息")
            columns = self.get_columns_of_record_table(bs_table, page, table_name)
            titles = [column[0] for column in columns]
    ...
    def parse_table_qygs_biangengxinxi(self, bs_table, table_name, page, post_data):
        ...
        sub_json_list = []
        try:
            settings.logger.info(u"parse qygs table 股东及出资- 变更信息 ")
            columns = self.get_columns_of_record_table(bs_table, page, table_name)
            titles = [column[0] for column in columns]
            ...
            settings.logger.error(u"parse qygs table 股东及出资信息- 变更信息 failed with exception:%s" % (type(e)))
        finally:
            return sub_json_list
    ...
    def parse_table_qygs_gudongchuzi(self, bs_table, table_name, page, post_data):
        ...
        sub_json_list = []
        try:
            settings.logger.info(u"parse qygs table 股东及出资信息 ")
    ...
    def parse_table_share_freeze(self, page, div_id=""):
        ...
        sub_json_list = []
        try:
            settings.logger.info(u"parse table 股东信息表")
