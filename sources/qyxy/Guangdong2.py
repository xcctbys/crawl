...
class Crawler(object):
        ...
        try:
            ...
            for tab, div_name in zip(tabs, div_names):
                ...
                if div_name == 'jibenxinxi':
                    ...
                    sub_json_dict['ind_comm_pub_reg_modify'] = dict_jiben[u'变更信息'] if dict_jiben.has_key(
                        u"变更信息") else {}
                    ...
                    sub_json_dict['ind_comm_pub_equity_ownership_reg'] = dj[u'股权出质登记信息'] if dj.has_key(
                        u'股权出质登记信息') else []
                    ...
                    dj = self.analysis.parse_page_2(page, div_name, post_data)
                    sub_json_dict['ind_comm_pub_administration_sanction'] = dj[u'行政处罚信息'] if dj.has_key(
                        u'行政处罚信息') else []
    ...
    def crawl_ent_pub_pages(self, url, types=2, post_data={}):
        sub_json_dict = {}
        try:
            page = self.crawl_page_by_url_post("http://gsxt.gdgs.gov.cn/aiccips/BusinessAnnals/BusinessAnnalsList.html",
                                               post_data)['page']
            ...
            page = self.crawl_page_by_url_post("http://gsxt.gdgs.gov.cn/aiccips/AppPerInformation.html",
                                               post_data)['page']
            ...
            page = self.crawl_page_by_url_post("http://gsxt.gdgs.gov.cn/aiccips/XZPunishmentMsg.html",
                                               post_data)['page']
            ...
            page = self.crawl_page_by_url_post("http://gsxt.gdgs.gov.cn/aiccips/ContributionCapitalMsg.html",
                                               post_data)['page']
            ...
            page = self.crawl_page_by_url_post(
                "http://gsxt.gdgs.gov.cn/aiccips/GDGQTransferMsg/shareholderTransferMsg.html", post_data)['page']
    ...
    def crawl_other_dept_pub_pages(self, url, types=2, post_data={}):
        ...
        try:
            ...
            page = self.crawl_page_by_url_post(
                "http://gsxt.gdgs.gov.cn/aiccips/OtherPublicity/environmentalProtection.html", post_data)['page']
    ...
    def crawl_judical_assist_pub_pages(self, url, types=2, post_data={}):
        ...
        try:
            ...
            #     sub_json_dict[title] = self.analysis.parse_page_2(page, ids, post_data)
            page = self.crawl_page_by_url_post(
                'http://gsxt.gdgs.gov.cn/aiccips/judiciaryAssist/judiciaryAssistInit.html', post_data)['page']
            ...
            page = self.crawl_page_by_url_post('http://gsxt.gdgs.gov.cn/aiccips/sfGuQuanChange/guQuanChange.html',
                                               post_data)['page']
        ...
        try:
            ...
            if columns:
                ...
                if len(tables) == 2 and tables[1].find('a'):
                    ...
                    res = self.crawler.crawl_page_by_url_post(
                        url, data, {'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8', })
                    #print res['page']
                    if table_name == u"变更信息":
                        ...
                        titles = [column[0] for column in columns]
                        for i, model in enumerate(d['list']):
                            data = [i + 1, model['regNO'], model['brName'].encode('utf8').decode('utf8'),
                        ...
                        d = json.loads(res['page'])
                        titles = [column[0] for column in columns]
                        surl = "http://gsxt.gdgs.gov.cn/aiccips/GSpublicity/invInfoDetails.html?" + "entNo=" + str(
                                ...
                                if td.find('a'):
                                    ...
                                    if re.match(r"http", next_url):
                                        ...
                                        if table_name == u'企业年报':
                                            ...
                                            print "in table_name"
                                            page_data = self.parse_ent_pub_annual_report_page_2(detail_page['page'],
                                                                                                table_name + '_detail')
                                            item[columns[col_count][0]] = self.get_column_data(columns[col_count][1],
                                                                                               td)
                                            item[u'详情'] = page_data
