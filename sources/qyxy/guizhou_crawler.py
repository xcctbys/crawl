...
class GuizhouCrawler(object):
    ...
    def help_get_dict_form_enter(self, lsh):
        ...
        result_dict = self.send_post_for_enter('http://gsxt.gzgs.gov.cn/nzgs/search!searchNbxx.shtml', self.nbxh, '0',
                                               '15', lsh)
        value = self.get_dict_enter(allths=[u'类型', u'名称', u'网址'], alltds=result_dict, alltds_keys=[], )
    ...
    def run(self, findCode):
        ...
        print result_dict
        self.get_json_one(allths=[u'注册号/统一社会信用代码', u'名称', u'类型', u'法定代表人', u'注册资本', u'成立日期', u'住所', u'营业期限自', u'营业期限至',
                                  u'经营范围', u'登记机关', u'核准日期', u'登记状态'],
                          alltds=result_dict,
                          alltds_keys=[u'zch', u'qymc', u'qylxmc', u'fddbr', u'zczb', u'clrq', u'zs', u'yyrq1',
                                       u'yyrq2', u'jyfw', u'djjgmc', u'hzrq', u'mclxmc'],
        ...
        print result_dict
        self.get_json_one(allths=[u'序号', u'登记编号', u'出质人', u'证照/证件号码', u'出质股权数额', u'质权人', u'证照/证件号码', u'股权出质设立登记日期',
                                  u'状态', u'公示日期', u'变化情况'],
                          alltds=result_dict,
                          alltds_keys=[],
        ...
        print result_dict
        self.get_json_one(allths=[], alltds=result_dict, alltds_keys=[], head='ind_comm_pub_administration_sanction')
        result_dict = self.send_post('http://gsxt.gzgs.gov.cn/nzgs/search!searchData.shtml', nbxh, '0', '33')
        print result_dict
        self.get_json_one(allths=[], alltds=result_dict, alltds_keys=[], head='ind_comm_pub_business_exception')
        result_dict = self.send_post('http://gsxt.gzgs.gov.cn/nzgs/search!searchData.shtml', nbxh, '0', '34')
        print result_dict
        self.get_json_one(allths=[], alltds=result_dict, alltds_keys=[], head='ind_comm_pub_serious_violate_law')
        result_dict = self.send_post('http://gsxt.gzgs.gov.cn/nzgs/search!searchData.shtml', nbxh, '0', '35')
        print result_dict
        self.get_json_one(allths=[], alltds=result_dict, alltds_keys=[], head='ind_comm_pub_spot_check')
        ...
        print result_dict
        self.get_json_two(allths=[u'股东', u'认缴额（万元）', u'实缴额（万元）', u'认缴出资方式', u'认缴出资额（万元）', u'认缴出资日期', u'认缴公示日期',
                                  u'实缴出资方式', u'实缴出资额（万元）', u'实缴出资日期', u'实缴公示日期'],
                          alltds=result_dict,
                          alltds_keys=[u'tzrmc', u'ljrje', u'ljsje', u'rjczfs', u'rjcze', u'rjczrq', u'rjgsrq',
                                       u'sjczfs', u'sjcze', u'sjczrq', u'sjgsrq'],
        ...
        print result_dict
        self.get_json_two(allths=[], alltds=result_dict, alltds_keys=[], head='ent_pub_shareholder_modify')
