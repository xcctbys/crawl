...
class AnhuiCrawler(object):
    ...
    def do_with_hasnext(self, mydict, head, table_head, table_next):
                        ...
                        for td in tr.find_all('td'):
                            if td.find('a'):
                                ...
                                if temp.status_code == 200:
                                    ...
                                    specially_dict = self.do_with_specially(mydict, head,
                                                                            detail_soup.find_all('table')[0])
                                else:
                                    print 'error...temp'
                            else:
        ...
        # 	pass
            ...
            for i in range(1, a_count + 1):
                try:
                    tempresp = self.reqst.get(
                        self.mydict['eareName'] + self.jsp_one_dict[head] + 'pno=' + str(i) + '&mainId=' + self.id,
                        timeout=120)
                except TypeError:
                    break
                if tempresp.status_code == 200:
    ...
    def do_with_enter(self, mydict, head, table_enter):
        ...
        for tr in trs[2:]:
            ...
            alltds = [self.help_enter_get_dict(self.get_tables(self.mydict['eareName'] + td.a['href'])) if td.find('a')
                      else td.get_text() for td in tr.find_all('td')]
            templist.append(self.get_one_to_one_dict(allths, alltds))
        self.result_json_dict[mydict[head]] = templist
        return templist
