...
from collector.models import (Job, CrawlerTask, CrawlerTaskGenerator, CrawlerGeneratorLog, CrawlerGeneratorErrorLog,
                              CrawlerGeneratorAlertLog, CrawlerGeneratorCronLog, CrawlerGeneratorDispatchLog)
...
class GeneratorQueue(object):
    ...
    def __init__(self,
                 redis_url=settings.GENERATOR_REDIS,
...
class GeneratorDispatch(object):
    ...
    def get_generator_object(self):
        try:
            generator_object = CrawlerTaskGenerator.objects(job=self.job_id,
                                                            status=CrawlerTaskGenerator.STATUS_ON).first()
        except Exception:
            ...
            if not queue.enqueue(priority, generate_uri_task, args=[generator_object]):
                if priority in (4, 5):
                    CrawlerGeneratorAlertLog(
                        name="QUEUES_FILLED_UP",
                        content="Low queues is full. Discard the generator(%s) from priority %d Exit!" % (
                            str(generator_object.id), generator_object.job.priority),
                        hostname=socket.gethostname()).save()
                    break
                else:
                ...
                content = "dispatch generator(%s)(%d) into queue with priority %d " % (
                    str(generator_object.id), generator_object.job.priority, priority)
                CrawlerGeneratorDispatchLog(job=generator_object.job,
                                            task_generator=generator_object,
...
def exec_command(command, comment, cron):
    ...
    CrawlerGeneratorCronLog(job=job,
                            ...
                            cron=cron,
                            failed_reason=failed_reason,
                            spend_msecs=int(1000 * (end_time - start_time))).save()
...
def force_exit():
    ...
    CrawlerGeneratorAlertLog(name="TIMEOUT",
                             content="Cron runtime exceeds 60s. Exit!",
                             hostname=socket.gethostname()).save()
    os.killpg(pgid, 9)
    os._exit(1)
