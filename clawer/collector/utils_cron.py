...
class CronTab(object):
    ...
    def new(self, command='', comment='', user=None):
        ...
        return self.append(CronItem(None, command, comment, datetime.now(), user, self))
...
class CronItem(object):
    ...
    def _set_parse(self, result):
        ...
        if self.cron.user == False:
            ...
            (self.user, cmd) = (result[0][-4] + ' ').split(' ', 1)
            self.set_command(cmd)
        else:
            self.set_command(result[0][-4])

        self.valid = self.setall(*result[0][:-4])
        self.comment = result[0][-1]
        self.last_run = result[0][-3][1:].strip()
    ...
    def render(self):
        ...
        if self.cron and self.cron.user is False:
            ...
            user = self.user + ' '
        result = u"%s %s%s @%s" % (str(self.slices), user, self.command, self.last_run)
        if self.comment:
