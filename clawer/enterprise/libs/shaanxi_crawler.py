    ...
    def crawl_page_main(self):
        ...
        try:
            for ent, url in self.ents.items():
                ...
                threads.append(gevent.spawn(self.crawl_ent_pub_pages, url, {'maent.pripid': pripid,
                                                                            'random': int(time.time())}))
                threads.append(gevent.spawn(self.crawl_other_dept_pub_pages, url, {'maent.pripid': pripid,
                                                                                   'random': int(time.time())}))
                threads.append(gevent.spawn(self.crawl_judical_assist_pub_pages, url, {'maent.pripid': pripid,
                                                                                       'random': int(time.time())}))
        ...
        if bs4_tag.has_attr('href') and (bs4_tag['href'] != '###' and bs4_tag['href'] != '#' and
            ...
            if re.compile(r'showRyxx').search(astr):
                ...
                if m:
                    ...
                    return {'method': 'POST',
                            'url': self.urls['host'] + "ztxy.do",
                            'data': {'method': 'tzrCzxxDetial',
                                     'maent.xh': ryId,
                                     "maent.pripid": nbxh,
