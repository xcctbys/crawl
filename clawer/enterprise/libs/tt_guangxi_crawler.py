...
class GuangxiCrawler(object):
                ...
                if resp.status_code == 200:
                    ...
                    if head == u'股东（发起人）信息' or head == u'股东信息' or head == u'动产抵押登记信息':
                        for td in next_table.find_all('td'):
                            if td.find_all('a'):
                                ...
                                if head == u'股东（发起人）信息' or head == u'股东信息':
                                    # print self.search_dict['eareName'] + detail_href[first+13:-2]
                                    alltds.append(self.get_dict_from_gudong_detail(self.get_tables(self.search_dict[
                                        'eareName'] + detail_href[first + 13:-2])[0]))
                                elif head == u'动产抵押登记信息':
                                    alltds.append(self.get_dict_from_dongcan_detail(self.get_tables(self.search_dict[
                                        'eareName'] + detail_href[first + 13:-2])))
                            elif td.get_text():
                        ...
                        if i == 1 and alltds:
                            ...
                            alltds = alltds + [
                                td.get_text().strip() if td.get_text() else None for td in next_table.find_all('td')
                            ]
                else:
                    pass
                    # print 'get____error'
        ...
        if head == u'股东（发起人）信息' or head == u'动产抵押登记信息' or head == u'股东信息':
            ...
            for td in table_td.find_all('td'):
                if td.find_all('a'):
                    detail_href = td.a['onclick']
                    first = detail_href.find('window.open')
                    # print detail_href[first+13:-2],head
                    if head == u'股东（发起人）信息' or head == u'股东信息':
                        # print self.search_dict['eareName'] + detail_href[first+13:-2]
                        alltds.append(self.get_dict_from_gudong_detail(self.get_tables(self.search_dict['eareName'] +
                                                                                       detail_href[first + 13:-2])))
                    elif head == u'动产抵押登记信息':
                        alltds.append(self.get_dict_from_dongcan_detail(self.get_tables(self.search_dict['eareName'] +
                                                                                        detail_href[first + 13:-2])))
                elif td.get_text():
                    alltds.append(td.get_text().strip())
                else:
    ...
    def run(self, findCode):
        ...
        print id_args
        if id_args and id_args.download_args.get('id'):
            self.result_json_dict = {}
            ...
            # self.json_dump_to_file('guangxi.json', {self.ent_number: self.result_json_dict})
            # self.result_json_dict_list.append({self.ent_number: self.result_json_dict})
            # print self.result_json_dict['ind_comm_pub_reg_basic']
            ...
            for div in BeautifulSoup(self.after_crack_checkcode_page, 'html.parser').find_all('div',
                                                                                              attrs={'class': 'list'}):
                if div.find_all('a'):
                    self.id = div.find_all('a')[0]['href'].split('?')[1]
                    self.ent_number = div.find_all('span')[0].get_text().strip()
                    self.enterprise_name = div.find_all('a')[0].get_text().strip()
                    args =  CrawlerDownloadArgs.objects.filter(register_number=self.ent_number)\
                      or CrawlerDownloadArgs.objects.filter(unifield_number=self.ent_number).first() \
                      or CrawlerDownloadArgs.objects.filter(enterprise_name=self.ent_number).first()
                    if args:
                        args.delete()
                    args = CrawlerDownloadArgs(province='guangxi',
                                               register_number=self.ent_number,
                                               unifield_number=self.ent_number,
                                               enterprise_name=self.enterprise_name,
                                               download_args={'id': self.id})
                    args.save()
                else:
                    continue
                resp = self.reqst.get('http://gxqyxygs.gov.cn/businessPublicity.jspx?' + self.id,
