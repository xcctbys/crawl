...
class GuangxiCrawler(object):
		...
		if head == u'股东（发起人）信息' or head == u'动产抵押登记信息' or head == u'股东信息':
			...
			for td in table_td.find_all('td'):
				if td.find_all('a'):
					...
					if head == u'股东（发起人）信息' or head == u'股东信息':
						# print self.search_dict['eareName'] + detail_href[first+13:-2]
						alltds.append(self.get_dict_from_gudong_detail(self.get_tables(self.search_dict['eareName'] + detail_href[first+13:-2])))
					elif head == u'动产抵押登记信息':
						alltds.append(self.get_dict_from_dongcan_detail(self.get_tables(self.search_dict['eareName'] + detail_href[first+13:-2])))
				elif td.get_text():
					alltds.append(td.get_text().strip())
	...
	def run(self, findCode):
		...
		if id_args and id_args.download_args.get('id'):
			...
			for div in BeautifulSoup(self.after_crack_checkcode_page, 'html.parser').find_all('div', attrs={'class':'list'}):
				if div.find_all('a'):
