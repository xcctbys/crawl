...
class MyParser(Parser):
    ...
    def get_result_json_for_register_info(self, json_data, *args, **kwargs):
        ...
        # 	print key, value
        ths = [u'注册号', u'名称', u'类型', u'执行事务合伙人', u'主要经营场所', u'合伙期限自', u'合伙期限至', u'经营范围', u'登记机关', u'核准日期', u'成立日期',
               u'登记状态']
    ...
    def get_result_json_for_corporate_annual_reports_info(self, json_data, *args, **kwargs):
        ...
        for i, item in enumerate(json_data):
            ...
            page = self.crawler.crawl_page_by_url_post(
                ...
                data=self.info.annual_reports_info_data)
            # print page
            ths = [u'注册号/统一社会信用代码', u'企业名称', u'企业联系电话', u'邮政编码', u'企业通信地址', u'企业电子邮箱', u'企业是否有投资信息或购买其他公司股权', u'企业经营状态',
                   u'企业经营状态', u'从业人数']
            tds = [u'REG_NO', u'CORP_NAME', u'TEL', u'ZIP', u'ADDR', u'E_MAIL', u'IF_INVEST', u'PRODUCE_STATUS',
                   u'IF_WEBSITE', u'NET_AMOUNT']
            ...
            tds = [u'TOTAL_EQUITY', u'SALE_INCOME', u'PROFIT_TOTAL', u'SERV_FARE_INCOME', u'PROFIT_RETA', u'TAX_TOTAL',
                   u'DEBT_AMOUNT']
            tds = [json.loads(page)[0].get(td) for td in tds]
            ...
            page = self.crawler.crawl_page_by_url_post(
                'http://www.jsgsj.gov.cn:58888/ecipplatform/nbServlet.json?nbEnter=true',
                data=self.info.annual_reports_security_info_data)
            # print page
            ths = []
            tds = []
            ...
            page = self.crawler.crawl_page_by_url_post(
                'http://www.jsgsj.gov.cn:58888/ecipplatform/nbServlet.json?nbEnter=true',
                data=self.info.annual_reports_record_info_data)
            # print page
            ths = []
            tds = []
            annual_dict[u'修改记录'] = dict(zip(ths, tds))
            annual_list.append({u'序号': str(i + 1),
                                u'报送年度': item.get('REPORT_RESULT'),
                                u'发布日期': item.get('REPORT_DATE'),
                                u'详情': annual_dict})
        return annual_list
...
class IndustrialPubliction(object):
    ...
    def get_regirster_info(self, *args, **kwargs):
        self.info.register_info_page = self.crawler.crawl_page_by_url_post(
            'http://www.jsgsj.gov.cn:58888/ecipplatform/inner_pb/pb_queryCorpInfor_gsRelease.jsp',
            data=self.info.industrial_page_info_post_data)
        page = self.crawler.crawl_page_by_url_post(
            'http://www.jsgsj.gov.cn:58888/ecipplatform/ciServlet.json?ciEnter=true',
            data=self.info.industrial_basic_info_post_data)
        self.info.result_json['ind_comm_pub_reg_basic'] = self.parser.get_result_json_for_register_info(json.loads(
            page)[0])
        # print self.info.result_json

        page = self.crawler.crawl_page_by_url_post(
            'http://www.jsgsj.gov.cn:58888/ecipplatform/ciServlet.json?ciEnter=true',
            data=self.info.industrial_shorehold_info_post_data)
        # print page
...
class EnterprisePubliction(object):
    ...
    def get_corporate_annual_reports_info(self, *args, **kwargs):
        page = self.crawler.crawl_page_by_url_post(
            'http://www.jsgsj.gov.cn:58888/ecipplatform/nbServlet.json?nbEnter=true',
            data=self.info.enterprise_annual_report_info_post_data)
        # print page
    ...
    def get_shareholder_contribution_info(self, *args, **kwargs):
        page = self.crawler.crawl_page_by_url_post(
            'http://www.jsgsj.gov.cn:58888/ecipplatform/nbServlet.json?nbEnter=true',
            data=self.info.enterprise_shorehold_and_give_info_post_data)
        # print page
...
class OtherDepartmentsPubliction(object):
    ...
    def get_administrative_licensing_info(self, *args, **kwargs):
        page = self.crawler.crawl_page_by_url_post(
            'http://www.jsgsj.gov.cn:58888/ecipplatform/manyCommonFnQueryServlet.json?query_xingzhengxuke=true',
    ...
    def get_administrative_punishment_info(self, *args, **kwargs):
        page = self.crawler.crawl_page_by_url_post(
            'http://www.jsgsj.gov.cn:58888/ecipplatform/manyCommonFnQueryServlet.json?query_xingzhengchufa=true',
            data=self.info.other_administrative_licensing_info_past_data)
        # print page
    ...
    def get_major_tax_list_info(self, *args, **kwargs):
        page = self.crawler.crawl_page_by_url_post(
            'http://www.jsgsj.gov.cn:58888/ecipplatform/manyCommonFnQueryServlet.json?majorTaxList=true',
            data=self.info.other_administrative_licensing_info_past_data)
        # print page
