    ...
    def add(self, filename):
        try:
            with codecs.open(filename, 'r+', 'GB18030') as f:
                ...
                line_count = 0
                try:
                    slice = Enterprise.objects.all().aggregate(Max('slice'))['slice__max']
                    slice += 1
                except:
                    slice = 0
                ...
                while True:
                    ...
                    line_count += 1
                    #line = _line.strip().split(";")[0]
                    line = _line.strip().replace('\"',"").split(";")[0]
                    fields = line.strip().split(",")
                    if len(fields) < 3:
                        failed += 1
                        continue
                ...
                return {"is_ok": True, "line_count": line_count, 'success': success, 'slice':slice, 'failed': failed, 'multiple': multiple}
        except Exception, e:
