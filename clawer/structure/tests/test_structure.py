...
from collector.models import (CrawlerTask, CrawlerDownloadData, CrawlerDownloadType, CrawlerDownload,
                              CrawlerDownloadSetting, CrawlerTaskGenerator)
from structure.structure import (StructureGenerator, ParserGenerator, QueueGenerator, ExtracterGenerator,
                                 ExecutionTasks, insert_test_data, empty_test_data, parser_func, Consts)
...
def insert_job(name, text, parser_text, settings):
    ...
    #Structure
    parser = Parser(parser_id=name, python_script=parser_text, update_date=django.utils.timezone.now())
    parser.save()
    structureconfig = StructureConfig(job_copy_id=job_mongodb.id,
                                      job=job_mysql,
                                      parser=parser,
...
class TestStructureGenerator(TestCase):
    ...
    def test_get_task_priority(self):
        for test_crawlertask in self.test_crawlertasks:
            ...
            self.assertIn(test_priority, (Consts.QUEUE_PRIORITY_TOO_HIGH, Consts.QUEUE_PRIORITY_HIGH,
                                          Consts.QUEUE_PRIORITY_NORMAL, Consts.QUEUE_PRIORITY_LOW))
...
class TestParserGenerator(TestCase):
    ...
    def test_assign_parse_task(self):
        ...
        test_parse_job_id = self.test_parsergenerator.assign_parse_task(test_priority, test_rawparser,
                                                                        test_crawlerdownloaddata)
...
class TestQueueGenerator(TestCase):
    ...
    def test_enqueue(self):
        test_priority = random.choice((Consts.QUEUE_PRIORITY_TOO_HIGH, Consts.QUEUE_PRIORITY_HIGH,
                                       Consts.QUEUE_PRIORITY_NORMAL, Consts.QUEUE_PRIORITY_LOW))
        ...
        test_crawlerdownloaddata = CrawlerDownloadData.objects().first()
        test_parse_job_id = self.test_queuegenerator.enqueue(test_priority,
                                                             test_parser_func,
                                                             args=[test_crawlerdownloaddata])
        self.assertIsNotNone(test_parse_job_id)
...
class TestExtracterGenerator(TestCase):
    ...
    def test_assign_extract_task(self):
        ...
        test_extract_job_id = self.test_extractergenerator.assign_extract_task(test_priority, test.extracter,
                                                                               test_crawleranalyzeddata)
