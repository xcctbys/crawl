...
class RawParser(object):
    ...
    def parser(self, crawlerdownloaddata):  
        data = crawlerdownloaddata.response_body
        url = crawlerdownloaddata.crawlertack.uri
        return self.text_analysis(data, url)

    # def text_analysis(self, html, url, keywords):
    def text_analysis(self, html, url=None):
        try:
...
if __name__ == '__main__':
    ...
    baiduparser = RawParser()
    with open('baidu.data') as f:
        for line in f:
            result = baiduparser.text_analysis(line)
            print 'result++++++++++++++'
            print result
            print 'result++++++++++++++'
            print '\n'
